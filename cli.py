...
app = typer.Typer(name="kilo-proxy", help="Fully OpenAI and Claude-compatible API proxy for Kilo")
console = Console()
...
def proxy(
    ...
    console.print(f"[blue]Claude-compatible API: http://{host}:{port}/v1/messages[/blue]")
...
def _install_windows_startup(config: Config):
    """Install Windows startup using a VBS wrapper for reliable boot execution."""
    startup_folder = (
    ...
    # Create a batch file that sets up environment and runs the server
    # This ensures the Python environment is properly initialized at boot
    kilo_proxy_dir = Path.home() / ".kilo-proxy"
    kilo_proxy_dir.mkdir(parents=True, exist_ok=True)

    log_dir = kilo_proxy_dir / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    startup_log = log_dir / "startup.log"

    # Create a wrapper batch file for robust startup
    batch_path = kilo_proxy_dir / "kilo_proxy_startup.bat"
    batch_content = f'''@echo off
REM Kilo Proxy Startup Script
REM Generated by kilo-proxy autolaunch command
...
echo [%DATE% %TIME%] Starting Kilo Proxy... >> "{startup_log}"

REM Run the server with full path to Python
"{exe_to_use}" -m kilo_proxy.server_runner --host {config.host} --port {config.port} 2>&1 >> "{startup_log}"

REM If we get here, something went wrong - log it
echo [%DATE% %TIME%] Server exited with error level %ERRORLEVEL% >> "{startup_log}"
'''

    # Create a VBS script to run the batch file hidden (no console window)
    vbs_path = startup_folder / "KiloProxy.vbs"
    vbs_content = f'''Set WshShell = CreateObject("WScript.Shell")
WshShell.Run """{batch_path}""", 0, False
'''
    ...
    try:
        ...
        # Write the VBS launcher to Startup folder
        vbs_path.write_text(vbs_content, encoding="utf-8")

        if vbs_path.exists() and batch_path.exists():
            console.print(
                "[green]Autolaunch installed! Kilo Proxy will start on login.[/green]"
            )
            console.print(f"[dim]Startup script: {batch_path}[/dim]")
            console.print(f"[dim]Launcher: {vbs_path}[/dim]")
            console.print(f"[dim]Startup log: {startup_log}[/dim]")
        else:
            console.print("[red]Failed to create startup files[/red]")
    except Exception as e:
        console.print(f"[red]Error installing autolaunch: {e}[/red]")


def _uninstall_windows_startup():
    """Remove Windows startup files."""
    startup_folder = (
        Path(os.environ.get("APPDATA", ""))
        / "Microsoft"
        / "Windows"
    ...
    # New VBS launcher location
    vbs_path = startup_folder / "KiloProxy.vbs"

    # Old shortcut location (for backward compatibility)
    ...
    try:
        ...
        if vbs_path.exists():
            vbs_path.unlink()
            console.print(f"[green]Removed: {vbs_path}[/green]")
            removed = True

        # Remove old shortcut (backward compatibility)
        if shortcut_path.exists():
            shortcut_path.unlink()
            console.print(f"[green]Removed: {shortcut_path}[/green]")
            removed = True

        # Remove batch file
        if batch_path.exists():
            batch_path.unlink()
            console.print(f"[green]Removed: {batch_path}[/green]")
            removed = True

        if removed:
            console.print("[green]Autolaunch removed![/green]")
        else:
            console.print("[yellow]No autolaunch files found[/yellow]")
    except Exception as e:
